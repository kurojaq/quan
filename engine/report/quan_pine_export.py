...
from quan_chronometric_objects import build_store, AnalysisResult
...
def analyze(csv_path, anchor, multiplier=20.0, T_days=1.0, window=400, max_strikes=14, greeks_csv=None, survivors=None):
    ...
    payload = (f"ANCHOR={anchor:.1f}|BIAS_VOTE={bias_vote}|CASCADE_VOTE={cas['BIAS']}|CDS={cas['CDS']:.2f}"
               ...
               f"|SNAP={','.join(snap)}"
               f"{survseg}")
    # Chronometric Store — second product of THIS SAME pass (consumes already-computed
    # fr/cas/rw/ts; never recomputes the payload). Defensive: a store failure must not break
    # the legacy payload interface.
    try:
        store = build_store(fr, anchor, cas, rw, ts=ts)
    except Exception as _e:
        store = {"error": "%s: %s" % (type(_e).__name__, _e)}
    return AnalysisResult(payload=payload, store=store)


def export_snapshot(*args, **kwargs):
    """Back-compat accessor: the canonical flat payload string only. ONE analytical pass via
    analyze(); the Chronometric Store is discarded here. Callers needing the store call
    analyze(...) and read .payload / .store off the single immutable AnalysisResult."""
    return analyze(*args, **kwargs).payload
