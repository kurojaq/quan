...
SCALP_PTS=25.0; SCALP_STOP=20.0   # true scalp: small fixed target + matched stop (~1.25:1)
...
def run_session(name, chain_csv, price_csv, d, require_gate=True, entry_mode="reclaim", exit_mode="runner"):
    ...
    # exit_mode decides which zones to target and how to scale out
    if exit_mode=="scalp":
        tgts = ahead[1:2] if len(ahead)>1 else []        # ONLY the adjacent next zone, full exit
        scale=[100]
    elif exit_mode=="scalp_fixed":
        tgts = []                                         # fixed-distance scalp set after entry (SCALP_PTS)
        scale=[100]
    elif exit_mode=="scalp_trail":
        tgts = ahead[1:4] if len(ahead)>1 else ahead[:3]  # next zone half, trail rest on later zones
        scale=[50,25,25]
    else:                                                 # runner
        tgts = ahead[1:4] if len(ahead)>1 else ahead[:3]
        scale=[34,33,33]
    if entry_zone is None or (not tgts and exit_mode!="scalp_fixed"):
    ...
    for (dt,o,h,l,c) in bars:
        ...
        if not entered and win_open and fold_ok:
            if entry_mode=="reclaim":
                ...
                if tested and held:
                    entry=c
                    if exit_mode=="scalp_fixed":
                        stop=entry-SCALP_STOP*cdir; tgts=[(entry+SCALP_PTS*cdir,0)]
                    else:
                        stop=(l if cdir>0 else h)-(STOP_BUFFER if cdir>0 else -STOP_BUFFER)
                    risk_pts=abs(entry-stop); qty=size_qty(risk_pts,use_tier,fstar)
                    if qty>0: avg=entry; qo=qty; qty0=qty; entered=True
        ...
        if qo>0:
            ...
            for ti,(tpx,mass) in enumerate(tgts,start=1):
                if ((h>=tpx) if cdir>0 else (l<=tpx)) and ti not in hit and qo>0:
                    pct=scale[ti-1] if ti-1<len(scale) else 0
                    q=min(int(round(qty0*pct/100)),qo) if ti<len(tgts) else qo   # last target takes the rest
                    q=max(q,1) if qo>0 else 0
                    exits.append((f"T{ti}",tpx,q)); qo-=q; hit.add(ti)
                    if exit_mode=="scalp_trail" and ti==1 and qo>0:
                        stop = avg + (STOP_BUFFER if cdir>0 else -STOP_BUFFER)*cdir  # trail to ~breakeven
            if qo<=0: break
...
def run_all(require_gate, entry_mode, exit_mode="runner"):
    mode=("GATED" if require_gate else "BIAS")+f" · {entry_mode}/{exit_mode}"
    print(f"\n{'='*84}\n  {mode}\n{'='*84}")
    print(f"{'sess':>6} {'anchor':>7} {'CDS':>5} {'state':>6} {'cwin':>5} {'tens':>5} {'entryZ':>7} {'stop':>6} {'targets':>18} {'trd':>4}")
    allt=[]
    for nm,ch,pr,d in SESSIONS:
        s,tr=run_session(nm,ch,pr,d,require_gate=require_gate,entry_mode=entry_mode,exit_mode=exit_mode)
    ...
    if allt:
        for t in allt:
            ...
            print(f"   {t['sess']} {'LONG' if t['dir']>0 else 'SHORT'} entry {t['avg']:.0f} stop {t['stop']:.0f} q{t['qty']}  R={t['R']:+.2f} ${t['pl']:+,.0f}  [{ex}]")
        ...
        print(f"  AGGREGATE: trades={n} wins={wins} hit={wins/n*100:.0f}%  totR={tR:+.2f}  P/L=${tpl:+,.0f}  PF={pf}")
    else: print("  no trades")
    return allt

if __name__=="__main__":
    for em in ["runner","scalp","scalp_fixed","scalp_trail"]:
        print(f"\n########## EXIT MODE: {em.upper()} (reclaim entry, BIAS view) ##########")
        run_all(False,"reclaim",em)
