...
def run(csv_path: str, anchor=None, T_days=1.0, instrument=None) -> dict:
    ...
    if anchor is not None:
        try:
            ...
            try:
                import quan_contracts as _CT
                _mult = _CT.resolve_multiplier(instrument)
            except Exception:
                _mult = 20.0
            sig["GREEKS"] = _GK.gex_read(frame, float(anchor), multiplier=_mult, r=0.053, T_days=T_days)
            sig["_multiplier"] = _mult
        except Exception as e:
            sig["GREEKS"] = None
