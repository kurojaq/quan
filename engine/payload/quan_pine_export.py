...
def export_snapshot(csv_path, anchor, multiplier=20.0, T_days=1.0, window=400, max_strikes=14, greeks_csv=None, survivors=None, just_expired=None):
    ...
    # SURVIVING LEVELS.
    # CANONICAL (role Y, 2026-06-20): SWALLS rides the just-expired + front pair already in the
    # session. The just-expired weekly is the FRONT (confirms persistence); THIS chain (the live,
    # longer-dated weekly) is the SURVIVOR (defines magnitude — its gamma is what's still standing
    # after the roll). confirmed_only=True -> emit ONLY the double-confirmed hot spots.
    #   SWALLS = surviving_levels(just_expired_gex, [this_chain_gex], confirmed_only=True)
    # Legacy `survivors` path (this chain as front, a SEPARATE longer-dated expiry as survivor) is
    # kept for back-compat but is NOT the session default; it inverts the roll and pulls in walls
    # from a non-session expiry. The |GEX| (gamma*OI) is anchor-independent, so anchor choice for
    # the just-expired book does not move the wall set.
    swall = ""
    def _gmap(L):
        return {round(float(s)): abs(float(v)) for s, v in zip(L.get("strikes", []), L.get("gex_perstrike", []))}
    if just_expired:
        try:
            jfr = ingest_chain(just_expired["chain"])
            if just_expired.get("greeks"):
                jgf = GK.load_greeks_csv(just_expired["greeks"])
            else:
                jgf = GK.greeks_from_chain(jfr, anchor, multiplier=multiplier, T_days=T_days)
            je_g = _gmap(GK.greeks_layer(jfr, jgf, anchor, multiplier=multiplier))
            surv_g = _gmap(greeks)  # THIS chain is the survivor
            swall = GK.surviving_levels(je_g, [surv_g], top=6, topk=8, confirmed_only=True)
        except Exception:
            swall = ""
    elif survivors:
