...
def _pressure_inputs(frame, anchor=None, anchor_idx=None):
    ...
    for i in range(N - 1):
        if np.isfinite(bi[i]) and np.isfinite(bi[i + 1]):
            br[i] = bi[i + 1] - bi[i]
    # AUTO ANCHOR: first row (top-down) where RIPN hits an extreme (0 or 1). Its strike location drifts
    # day to day (often far OTM, esp. equities) — that drift IS the observed phenomenon, kept as-is.
    auto_ai = None
    for i in range(N):
        if bi[i] == 0.0 or bi[i] == 1.0:
            auto_ai = i; break
    # MANUAL HANDSHAKE (Quanyun, 2026-06-26): the operator-selected RIPN row OVERRIDES the auto pick as the
    # derivative anchor — restoring the Golden Reference handshake into the terminal. auto_ai is still
    # computed/returned so the UI can show the system's first guess vs the operator's choice. When no valid
    # override is given the behavior is byte-identical to the old auto-only path.
    ai = auto_ai
    manual = False
    if anchor_idx is not None:
        try:
            ...
            _aidx = None
        if _aidx is not None and 0 <= _aidx < N and np.isfinite(bi[_aidx]):
            ai = _aidx; manual = True
    if ai is None:
        return None
    ...
    # RIPN HANDSHAKE inspection rows for the terminal panel (mirrors the Golden Reference operator view):
    # [row_idx, strike, RIPN(bi)[0,1], AP (raw intent pressure), tuning(BR = bi[r+1]-bi[r])].
    ripn_rows = []
    for r in range(N):
        ripn_rows.append([
            r, float(strikes[r]),
            (float(bi[r]) if np.isfinite(bi[r]) else None),
            (float(ap[r]) if np.isfinite(ap[r]) else None),
            (float(br[r]) if (r < len(br) and np.isfinite(br[r])) else None),
        ])
    return cw, cc, cd, dict(anchor_strike=float(strikes[ai]), anchor_idx=ai,
                            ...
                            auto_idx=(auto_ai if auto_ai is not None else -1),
                            used_idx=ai, manual_anchor=manual, ripn_rows=ripn_rows,
...
def realization_waves(frame, anchor, anchor_idx=None):
    ...
    anchor_idx (optional): operator's RIPN-handshake row; overrides the auto derivative anchor when valid."""
    pin = _pressure_inputs(frame, anchor, anchor_idx)
    if pin is None:
        return None
    cw, cc, cd, meta = pin
